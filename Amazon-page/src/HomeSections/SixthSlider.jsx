...
import SixthSliderData from "./SixthSliderData";

const SixthSlider = () => {
  const navigate = useNavigate();
  const [index, setIndex] = useState(0);
  const [visibleItems, setVisibleItems] = useState(8); // Default Desktop

  // --- 1. Responsiveness: Screen size ke mutabiq items count change hoga ---
  useEffect(() => {
    const handleResize = () => {
  ...
  const maxIndex = Math.max(0, SixthSliderData.length - visibleItems);
  ...
  // --- 2. Navigation Logic: Click par data detail page par bhej rahy hain ---
  const handleItemClick = (item) => {
    // '/product/:id' path par navigate aur 'item' object ko state mein pass karna
  ...
  return (
    <div className="relative bg-white w-[96%] mx-auto shadow-md mt-5 overflow-hidden py-4">
      ...
      {/* Left Arrow Button (Boundary check ke sath) */}
      {index > 0 && (
        <button
          onClick={prevSlide}
      ...
      >
        ...
        >
          {SixthSliderData.map((item) => (
            ...
            >
              {/* Individual image transition logic */}
              <img
                src={item.image}
                alt=""
      ...
      {/* Right Arrow Button (Boundary check ke sath) */}
      {index < maxIndex && (
        <button
          onClick={nextSlide}
