...
import FifthCardData from "../HomeSections/FifthCard";

const FifthSlider = () => {
  const navigate = useNavigate();
  const [index, setIndex] = useState(0);
  const [visibleItems, setVisibleItems] = useState(7); // Default Desktop ke liye
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      else setVisibleItems(7);                    // Desktop
    };
  ...
  const maxIndex = Math.max(0, FifthCardData.length - visibleItems);
  ...
  // --- 2. Navigation Logic: Item click par next page par data bhej raha hai ---
  const handleItemClick = (item) => {
    // '/product/:id' par navigate aur 'item' object ko state mein pass kar rahe hain
  ...
  return (
    <div className="relative bg-white w-[96%] mx-auto shadow-md overflow-hidden py-4">
      ...
      {/* Left Navigation Button (Conditions ke sath) */}
      {index > 0 && (
        <button
          onClick={prevSlide}
      ...
      >
        ...
        >
          {FifthCardData.map((item) => (
            ...
            >
              {/* Transition image par alag se lagayi hai */}
              <img
                src={item.image}
                alt=""
      ...
      {/* Right Navigation Button (End logic ke sath) */}
      {index < maxIndex && (
        <button
          onClick={nextSlide}
