...
const SecondProductSlider = ({ 
  ...
  const updateCount = useCallback(() => {
    ...
    else if (w < 1440) setVisibleCount(6.2); 
  ...
  return (
    <div 
      className="relative bg-white w-[96%] mx-auto py-8 px-4 md:px-8 border-b border-gray-100 group/main select-none mb-5"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="flex items-end justify-between mb-6 px-2">
        <div className="space-y-1">
          <h2 className="text-xl md:text-2xl font-bold text-[#111] tracking-tight">{title}</h2>
              ...
              <div className="flex flex-col bg-white rounded-xl transition-all duration-500 hover:-translate-y-2 active:scale-95">
                ...
                <div className="relative aspect-square w-full bg-[#f8f9fa] rounded-xl overflow-hidden flex items-center justify-center p-6 border border-transparent group-hover/item:border-gray-100 group-hover/item:shadow-lg transition-all duration-500">
                  <img
                    ...
                    alt={item.title || "wireless product"}
                  />
                  
                  {item.price > 200 && (
                    <div className="absolute top-3 right-3 bg-gray-900 text-white text-[8px] font-bold px-2 py-0.5 rounded-full">
                      PREMIUM
                    </div>
                  )}
                </div>
                ...
                {item.title && (
                    <div className="mt-3 px-1">
                        <p className="text-xs font-medium text-gray-700 line-clamp-1 group-hover/item:text-blue-600 transition-colors">
                            {item.title}
                        </p>
                    </div>
                )}
